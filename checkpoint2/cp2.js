...
const distroObj = (nums) => {
  const newObj = {};
  nums.forEach((item, index) => {
    //check if the key with item already exists
    //then increamnet the value of the key
    //if not create the key with value one
...
console.log(distroObj(nums));

// Find the index of the first "Austin" value (the value plus its index equals 512)

const findAtx = (nums) => {
  let atxIdx = 0;
  //loop for all entries and get element and index
  nums.every(function(element, index) {
    //when sum of element+index is 512, store the index and
